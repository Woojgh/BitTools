...
function addInputs(choices) {
  ...
  for (var i = 0; i < parseInt(choices.value); i++) {
    ...
    wrapper.innerHTML = '<input class="fillcheckbox" type="checkbox"> <input class="choice-input" type="text"  style="margin:1vw; color:colorValue;" placeholder="choice ' + i + '" /> <input class="fill-color" value="#00FFFF" type="color" name="barfillcolor">';
...
function addPreview(choices) {
  ...
  for (var i = 0; i < parseInt(choices.value); i++) {
    var wrapper = document.createElement('div');
    wrapper.innerHTML = '<span>0</span> <a href=" ">Vote</a>';
    result.appendChild(wrapper);
  }
};
